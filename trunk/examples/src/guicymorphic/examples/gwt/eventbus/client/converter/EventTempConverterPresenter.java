...
import guicymorphic.examples.gwt.eventbus.client.EventBusExample;
 ...
 * @author Alen Vrecko
 */
public class EventTempConverterPresenter {
    ...
    public EventTempConverterPresenter(EventBus bus, BrowserHistory history) {
        this.bus = bus;
    ...
    public void fahrenheitValueChanged(String value) {
        try {
            ...
            // Notice we don't fire the history event but event
            // as a rule of thumb fire history event when nobody depends on your view/presenter
            // fire event when somebody depends on you this will put excess garbage at a minimum
            bus.fireEvent(new ConversionEvent(celsius, fahrenheit));
            history.newItem(EventBusExample.CONVERT, "c", celsius, "f", fahrenheit);
